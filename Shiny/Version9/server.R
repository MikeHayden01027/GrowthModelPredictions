...
load('westBrookPred2.RData') 
 ...
 p2 <- p2 %>% rename(predGr = predGrowTH , Species = Speies) 
...
shinyServer(function(input, output) {
  ...
  output$plot1A <- renderPlot({  
    ...
    data() %>% filter( type == 'temp' ) %>% 
         ...
         #theme(legend.key = element_rect(colour = "black")) +
          labs(y = iterdataT , x = "Temp") + facet_grid(.~isYOY)})
  ...
  output$plot2A <- renderPlot({  
    ...
    data() %>% filter( type == 'flow' ) %>% 
      ...
      guides(fill = FALSE) +
      #theme(legend.key = element_rect(colour = "black")) +
      labs(y = iterdataT , x = "Stream Flow")+ facet_grid(.~isYOY)})
  ...
  output$plot3A <- renderPlot({  
    ...
    data() %>% filter( type == 'count' ) %>% 
      ...
      guides(fill = FALSE) +
      #theme(legend.key = element_rect(colour = "black")) +
      labs(y = iterdataT , x = "Estimated Abundance")+ facet_grid(.~isYOY)})
  ...
  output$plot4A <- renderPlot({  
    ...
    data() %>% filter( type == 'length' ) %>% 
      ...
      guides(fill = FALSE) +
      #theme(legend.key = element_rect(colour = "black")) +
      labs(y = iterdataT , x = "Length")+ facet_grid(.~isYOY)})
